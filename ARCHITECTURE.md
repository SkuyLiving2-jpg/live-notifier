...
| `daily-log.json` | `storage/dailyLog.js` | `{ date, sessions: [{ name, username, startedAtUnix, endedAtUnix, durationMs, peakViewCount }], recapSentDate }` | Every session (live or ended) for the current WIB day. On a date rollover, completed sessions from the old day are dropped, but any still-ongoing session (`endedAtUnix === null`) is carried over into the new day instead of being lost — a live that started before midnight and is still going stays visible in `cok rekap` instead of disappearing until it ends. Powers `cok rekap`, `cok paling lama/rame`. |
...
| `tests/dailyLog.test.js` | The peak-viewer-merge logic (the `0`-is-not-"no data" fix), the day-rollover reset (dropping completed sessions but carrying over still-ongoing ones), and the "session started before a restart" fallback reconstruction. |
...
**A second bug was reported by the owner directly** (a member who was still live wasn't showing up in `cok rekap`): `loadDailyLog()`'s day-rollover branch dropped *every* session once the WIB date changed, including a session still in progress (`endedAtUnix === null`). A live that started before midnight and was still ongoing after it would vanish from that day's recap entirely — not even showing as "Live" — until it finished, at which point `recordLiveEndedToday()`'s fallback path would reconstruct it from scratch. Fixed by carrying over only still-ongoing sessions across the rollover (completed ones are correctly dropped, since that day's recap already accounted for them); guarded by `tests/dailyLog.test.js`'s two carry-over tests.
